...
namespace app {
    std::thread module_post_process_face(int npu_h, int npu_w, Queue &queue_in, Queue &queue_out, std::atomic<bool> &push_on) {
    ...
    std::thread module_post_process_od(int npu_h, int npu_w, Queue &queue_in, Queue &queue_out, std::atomic<bool> &push_on) {
	auto func = [&, npu_h, npu_w]() {
	    ...
	    mobilint::post::YOLOv8PostProcessor post(od_nc, od_imh, od_imw, od_conf_thres, od_iou_thres, false);
		...
		uint64_t ticket =
		    post.enqueue(data_in.frame, data_in.npu_result, boxes, scores, labels, keypoints);
		post.receive(ticket);
    ...
    std::thread module_post_process_pose(int npu_h, int npu_w, Queue &queue_in, Queue &queue_out, std::atomic<bool> &push_on) {
	auto func = [&, npu_h, npu_w]() {
	    ...
	    mobilint::post::YOLOv8PosePostProcessor post(pose_nc, pose_imh, pose_imw, pose_conf_thres, pose_iou_thres, false);
		...
		std::vector<int> labels;
		std::vector<std::vector<float>> keypoints;
		uint64_t ticket = post.enqueue(data_in.frame, data_in.npu_result, boxes, scores, labels, keypoints);
