...
namespace gst_wrapper {
    AppsrcKmssinkPipeline::AppsrcKmssinkPipeline(const char* format_in, uint32_t width_in, uint32_t height_in, uint32_t fps_in) {
	create_elements(format_in, width_in, height_in, fps_in);
	link_elements(format_in, width_in, height_in, fps_in);
    }

    void AppsrcKmssinkPipeline::create_elements(const char* format_in, uint32_t width_in, uint32_t height_in, uint32_t fps_in) {
	/* create appsrc */
	...
	caps_appsrc_v4l2convert = gst_caps_new_simple("video/x-raw",
						      "format", G_TYPE_STRING, format_in,
						      "width", G_TYPE_INT, width_in,
						      "height", G_TYPE_INT, height_in,
						      "framerate", GST_TYPE_FRACTION, fps_in, 1,
						      NULL);
    ...
    void AppsrcKmssinkPipeline::link_elements(const char* format_out, uint32_t width_out, uint32_t height_out, uint32_t fps_out) {
	/* link appsrc, kmssink */
	GstCaps *caps_appsrc_kmssink;
	caps_appsrc_kmssink = gst_caps_new_simple("video/x-raw",
						  "format", G_TYPE_STRING, format_out,
						  "width", G_TYPE_INT, width_out,
						  "height", G_TYPE_INT, height_out,
						  "framerate", GST_TYPE_FRACTION, fps_out, 1,
						  NULL);
